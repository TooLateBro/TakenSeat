...
public class UserControllerV1 implements UserControllerDocs {
    ...
    public ResponseEntity<ApiResponseData<UserInfoResponseDtoV1>> getUserDetails(@PathVariable UUID userId,
                                                                                 @RequestParam(defaultValue = "0") int page,
                                                                                 @RequestParam(defaultValue = "10") int size) {

        UserInfoResponseDtoV1 userInfo = userServiceV1.getUserDetails(userId, page, size);
        return ResponseEntity.ok(ApiResponseData.success(userInfo));
    }
    ...
    public ResponseEntity<ApiResponseData<UserInfoResponseDtoV1>> updateUser(@PathVariable UUID userId,
        ...
        UserInfoResponseDtoV1 userInfo = userServiceV1.updateUser(userId, requestDto.toDto());
        return ResponseEntity.ok(ApiResponseData.success(userInfo));
    }
